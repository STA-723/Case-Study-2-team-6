...
To assess features of popular listings, we consider a log-linear model
$$
  \log(\texttt{monthly_reviews}) \sim  f_1(\texttt{price}) + f_2(\texttt{dist_to_subway}) + f_3(\texttt{months_active}) + \\
  \texttt{room_type} + \texttt{gender} + \texttt{private} + \cdots + \texttt{cozy} +\\
  \texttt{neighbourhoods}
$$
where $f_1, f_2 and f_3$ are increasing functions.

- This allows us to estimate trends in an interpretable way, while controlling for neighborhood effects.

- We tried estimating $f_1, f_2, f_3$ using shape-constrained generalized additive model packages (\texttt{cgam} and \texttt{scam}), but this didn't run in finite time.

- Instead we used boxcox transforms of the predictors, as implemented in the \texttt{car} package.
...
model.bx = lm(log(nrml_avg_reviews) ~ bx_fun(price, 0.57422) + bx_fun(dist_to_subway, 0.83087) + bx_fun(months_active, -0.27567) + room_type + gender + whole_price + private + square + minute + close + stock + apt + sunny + spacious + deluxe + newly + walking + cozy + central + fast + huge + west + green + neighbourhood, data=data)
...
```{r, fig.width=4, fig.height=3, fig.align="center"}
keywords = c("private", "square", "minute", "close", "stock", "apt", "sunny", 
