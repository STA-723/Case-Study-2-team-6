...
$$
  \log(\texttt{monthly reviews}) \sim  f_1(\texttt{price}) + f_2(\texttt{dist to subway}) + f_3(\texttt{months active})
$$
...
- We tried estimating $f_1$, $f_2$, $f_3$ using shape-constrained generalized additive model packages (cgam and scam), but this didn't run in finite time.

- Instead we used boxcox transforms of the predictors, as implemented in the car package.
